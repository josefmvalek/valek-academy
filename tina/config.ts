...
const scheduleField: any = {
  ...
  fields: [
    ...
    { type: "string", name: "smartCohort3Title", label: "Vyhledávač: Ročník 3 – Název (např. 7.–9. třída & SŠ)" },
    ...
    collections: [
      {
        ...
        fields: [
            ...
            fields: [
              ...
              { type: "string", name: "ctaSecondaryLink", label: "Odkaz sekundárního tlačítka" },
              {
                ...
                fields: [
                  ...
                  { type: "string", name: "teacherTag", label: "Specializace (např. Čistá výslovnost rodilého mluvčího & výuka hrou)" },
          ...
          },
          {
            ...
            fields: [
              ...
              },
              {
                type: "object",
                name: "stats",
                label: "Číselné odznaky / statistiky lektora",
                list: true,
                ui: {
                  itemProps: (item: any) => ({ label: `📊 ${item?.target || ""}${item?.suffix || ""} – ${item?.label || "Statistika"}` }),
                },
                fields: [
                  { type: "string", name: "target", label: "Cílové číslo (např. 25, 5, 60, 100)" },
                  { type: "string", name: "suffix", label: "Přípona čísla (např. +,  dětí,  min, %)" },
                  { type: "string", name: "label", label: "Popisek pod číslem (např. let zkušeností v ČR)" },
                  {
                    type: "string",
                    name: "colorScheme",
                    label: "Barevné schéma",
                    options: [
                      { label: "Jantarová (amber)", value: "amber" },
                      { label: "Tmavá (espresso)", value: "espresso" },
                      { label: "Smaragdová (emerald)", value: "emerald" },
                    ],
                  },
                ],
              },
          ...
          },
          {
            type: "object",
            name: "groupMatcher",
            label: "🎲 3krokový průvodce výběrem skupinky (Group Matcher)",
            fields: [
              { type: "boolean", name: "enabled", label: "Zobrazit 3krokového průvodce na webu?" },
              { type: "string", name: "badge", label: "Odznáček" },
              { type: "string", name: "title", label: "Hlavní nadpis" },
              { type: "string", name: "subtitle", label: "Podnadpis", ui: { component: "textarea" } },
              { type: "string", name: "step1Title", label: "Krok 1 - Nadpis (Ročník)" },
              { type: "string", name: "step2Title", label: "Krok 2 - Nadpis (Zájmy)" },
              { type: "string", name: "step3Title", label: "Krok 3 - Nadpis (Cíl/Potřeba)" },
              { type: "string", name: "ticketBadge", label: "Zlatá vstupenka - Odznak" },
              { type: "string", name: "availabilityBadge", label: "Zlatá vstupenka - Dostupnost" },
              { type: "string", name: "gamesLabel", label: "Zlatá vstupenka - Nadpis her" },
              { type: "string", name: "guaranteeText", label: "Zlatá vstupenka - Garance 1. lekce" },
              { type: "string", name: "scheduleLinkText", label: "Odkaz na rozvrh hodin (např. Zobrazit volné dny v rozvrhu ↓)" },
              { type: "string", name: "whatsappText", label: "Text tlačítka WhatsApp" },
              { type: "string", name: "ctaText", label: "Text hlavního tlačítka přihlášky" },
              {
                type: "object",
                name: "grades",
                label: "Ročníky dětí (Krok 1)",
                list: true,
                ui: {
                  itemProps: (item: any) => ({ label: `${item?.icon || "🎒"} ${item?.title || "Ročník"}` }),
                },
                fields: [
                  { type: "string", name: "id", label: "ID ročníku (1-3, 4-6, 7-9, ss)" },
                  { type: "string", name: "icon", label: "Ikona / Emoji" },
                  { type: "string", name: "title", label: "Název ročníku (např. 1.–3. třída ZŠ)" },
                  { type: "string", name: "age", label: "Věk (např. Věk 6–9 let)" },
                  ...
                  { type: "string", name: "recommendedGames", label: "Doporučené deskovky", list: true },
                ],
              },
              {
                type: "object",
                name: "interests",
                label: "Zájmy a deskovky (Krok 2)",
                list: true,
                ui: {
                  itemProps: (item: any) => ({ label: `${item?.icon || "🎲"} ${item?.title || "Zájem"}` }),
                },
                fields: [
                  { type: "string", name: "id", label: "ID zájmu (fantasy, words, speed, strategy)" },
                  { type: "string", name: "icon", label: "Ikona / Emoji" },
                  { type: "string", name: "title", label: "Název zájmu" },
                  { type: "string", name: "desc", label: "Popis her" },
                ],
              },
              {
                type: "object",
                name: "goals",
                label: "Cíle doučování (Krok 3)",
                list: true,
                ui: {
                  itemProps: (item: any) => ({ label: `${item?.icon || "🎯"} ${item?.title || "Cíl"}` }),
                },
                fields: [
                  { type: "string", name: "id", label: "ID cíle (confidence, grades, fun)" },
                  { type: "string", name: "icon", label: "Ikona / Emoji" },
